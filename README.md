...
Recommendation system for high school students seeking bachelor's scholarships abroad. Uses a two-tower neural network with a two-stage inference pipeline: hard filter (deterministic) → two-tower similarity scoring (neural network).
...
| `Scholarship` | Scholarship opportunity | ~47 |
| `Pair` | Student-scholarship pair with `relevance_score` label (regression target) | ~140,000 |
| `Feedback` | Implicit feedback (apply/click/view/reject) — reserved for future retraining on real data | ~100,000+ |
...
class Feedback:
    ...
    # Derived property:
    weight: float         # Signal weight (see table below) — reserved for future retraining
```
...
The two-tower neural network is trained as a **regression model** that predicts continuous `relevance_score` (0.0–1.0). Training uses a **Weighted MSE** loss that assigns higher weight to high-relevance pairs, compensating for the natural imbalance in the dataset (Match pairs are rare).
...
The model is initially bootstrapped on synthetic data generated by `generator_two_tower.py`. When real user interaction data (Feedback) is available, the model is retrained from scratch on real labels.

### Model Architecture

Each tower processes two input streams in parallel, then combines them:

```
Student Tower                          Scholarship Tower
─────────────────────────────          ─────────────────────────────
structured features                    structured features
  (numerical, one-hot, multi-hot)        (numerical, one-hot, multi-hot)
        │                                       │
      Dense                                   Dense
        │                                       │
        ├──────────────────────────────────────┤
...
text tokens                            text tokens
  (personal_statement,                   (mission_statement,
   ...
   future_goals)                               │
        │                              TextEncoderBlock
  TextEncoderBlock ◄── Shared Embedding Matrix ──►
        │                                       │
     concat([struct_vec, text_vec])          concat([struct_vec, text_vec])
        │                                       │
   Dense layers                           Dense layers
        │                                       │
   student_emb (d=128)                scholarship_emb (d=128)
```

**TextEncoderBlock** is a custom Keras layer wrapping `Shared Embedding Matrix + GlobalAveragePooling1D + Dense`. Both towers share the same embedding matrix so identical words produce identical token vectors.

**Output**: `sigmoid(student_emb · scholarship_emb)` → predicted `relevance_score` ∈ (0, 1)

### Inference Pipeline

```
┌─────────────────────────────────────────────┐
│  Stage 1: Hard Filter (Python function,     │
│           outside model)                    │
│  ─────────────────────────────────────────  │
...
│  ─────────────────────────────────────────  │
│  Student Tower  →  student_emb  (d=128)     │
│  Scholarship Tower → scholarship_emb (d=128)│
│                                             │
│  sigmoid(student_emb · scholarship_emb)     │
│                                             │
│  Output: predicted relevance_score (0, 1)  │
└──────────────────┬──────────────────────────┘
                   ↓
┌─────────────────────────────────────────────┐
