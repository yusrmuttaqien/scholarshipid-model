"""CSV serialization for the two-tower dataset.

Pure functions that convert domain dataclasses to CSV-compatible dicts,
plus save_to_csv which writes all four datasets to disk.
"""
...
from src.schemas import Feedback, Pair, Scholarship, Student
...
def save_to_csv(
    ...
    pd.DataFrame([flatten_pair(p) for p in pairs]).to_csv(
        f"{output_dir}/pairs.csv", index=False
    )
